...
public class MainVerticle extends AbstractVerticle {
  ...
  private static final String _SUPPORT_EXTS_PATTERN = "(.html|.htm|.json|.xml)?";
  private static final String _SUPPORT_EXTS_PATTERN2 = "^.*(\\.html|\\.htm|\\.json|\\.xml)$";
  private static final String _SUPPORT_RESOURCE_EXTS_PATTERN = "^.*(\\.jpg|\\.jpeg|\\.png|\\.gif|\\.css|\\.js|\\.ppt|\\.pdf|\\.png)$";
...
//    SessionStore sessionStore = LocalSessionStore.create(vertx, "myapp.sessionmap", 10000);
    ...
    vertxRouter.route().handler(ResponseTimeHandler.create());
//    vertxRouter.route().handler(ResponseContentTypeHandler.create());
    ...
    vertxRouter.route().handler(BodyHandler.create().setBodyLimit(50 * MB).setUploadsDirectory("uploads"));                        // file upload and body parameters parser
    vertxRouter.route().handler(SessionHandler.create(sessionStore).setCookieHttpOnlyFlag(false));
...
//    vertxRouter.route().handler(CSRFHandler.create("not a good secret"));
    ...
    vertxRouter.routeWithRegex(HttpMethod.POST, "/login-auth")
      .handler(CSRFHandler.create("csrf secret here"));

    vertxRouter.routeWithRegex(HttpMethod.GET, "/login" + _SUPPORT_EXTS_PATTERN)
      .handler(CSRFHandler.create("csrf secret here"))
    ...
    vertxRouter.route("/*").handler(this::notFoundHandler);
